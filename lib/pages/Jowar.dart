...
class RateConsole2 extends StatefulWidget {
  const RateConsole2({Key? key}) : super(key: key);

  @override
  _RateConsole2State createState() => _RateConsole2State();
}

class _RateConsole2State extends State<RateConsole2> {
  ...
  String? validateCropName(String? value) {
    if (value != null && value.toLowerCase() != 'jowar') {
      return 'Only Jowar crop can be added here.';
    }
    return null;
  }
  ...
  Future<void> _createRates() async {
    if (_formKey.currentState?.validate() ?? false) {
      await FirebaseFirestore.instance.collection('JowarRates').add({
        'name': _cropName,
        'price': _cropPrice,
  ...
  Future<void> _updateRates(String ratesId, String newRatesName,
    ...
    await FirebaseFirestore.instance.collection('JowarRates')
        .doc(ratesId)
        .update({
      'name': newRatesName,
      'price': newRatesPrice,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Jowar Rates Console'),
        backgroundColor: Colors.lightGreen[50],),
                                          ...
                                          children: [
                                            TextFormField(
                                              ...
                                              onChanged: (value) =>
                                                  setState(() =>
                                                  _newCropName = value),
                                              enabled: false, // Disable editing for crop name
                                            ),
                                            TextFormField(
